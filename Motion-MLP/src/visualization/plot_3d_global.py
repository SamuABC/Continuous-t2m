...
def plot_3d_motion_with_start(args, figsize=(10, 10), fps=120, radius=4):
    matplotlib.use("Agg")
    ...
    def update(index):
        ...
        if nb_joints == 21:
            fig = plt.figure(figsize=((2 * 480) / 96.0, 320 / 96.0), dpi=96)
        else:
            fig = plt.figure(figsize=(20, 10), dpi=96)

        ax_static = fig.add_subplot(121, projection="3d")
        ax_static.set_title("Start Frame", fontsize=12, y=0.95)

        ax_anim = fig.add_subplot(122, projection="3d")
        ax_anim.set_title(f"Frame {index}", fontsize=12, y=0.95)

        def plot_skeleton(ax, frame_idx, show_traj=False):
            init(ax)
            ...
            # Zoom logic (lower = closer)
            ax.dist = 0.0

            plot_xzPlane(
                ax,
                MINS[0] - trajec[frame_idx, 0],
                MAXS[0] - trajec[frame_idx, 0],
                0,
                MINS[2] - trajec[frame_idx, 1],
                MAXS[2] - trajec[frame_idx, 1],
            )

            if show_traj and frame_idx > 1:
                ax.plot3D(
                    trajec[:frame_idx, 0] - trajec[frame_idx, 0],
        ...
        plot_skeleton(ax_static, 0, show_traj=False)
        plot_skeleton(ax_anim, index, show_traj=True)

        plt.subplots_adjust(top=0.95, bottom=0.05, left=0.0, right=1.0, wspace=0.0)
