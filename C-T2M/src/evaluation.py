""" Modified from Guo et al. T2M evaluation code """
...
import numpy as np
import torch
from guoevaluation import paramUtil
...
from guoevaluation.metrics import *
...
def plot_t2m(data, save_dir, captions):
    ...
    for i, (caption, joint_data) in enumerate(zip(captions, data)):
        joint = recover_from_ric(
            torch.from_numpy(joint_data).float(), wrapper_opt.joints_num
        ).numpy()
        save_path = pjoin(save_dir, "%02d.mp4" % (i))
        plot_3d_motion(
            save_path, paramUtil.t2m_kinematic_chain, joint, title=caption, fps=20
        )
...
def evaluation(log_file):
    with open(log_file, "w") as f:
        ...
        for replication in range(replication_times):
            ...
            print(f"Time: {datetime.now()}")
            print(f"Time: {datetime.now()}", file=f, flush=True)
            div_score_dict = evaluate_diversity(
                acti_dict, f, diversity_times=diversity_times
            )

            # print(f'Time: {datetime.now()}')
            # print(f'Time: {datetime.now()}', file=f, flush=True)
            # mm_score_dict = evaluate_multimodality(eval_wrapper, mm_motion_loaders, f)
...
def animation_4_user_study(save_dir):
    ...
    for motion_loader_name, motion_loader in motion_loaders.items():
        for idx, batch in enumerate(motion_loader):
            ...
            plot_3d_motion(
                pjoin(ani_save_path, "%s.mp4" % (motion_loader_name)),
...
if __name__ == "__main__":
    # note: because the model is deterministice, multimodality is not evaluated here.
    # Therefore:
    # - replication_times is set to 1
    # - mm_motion_loaders are set to None in eval_motion_loaders
    # - evaluate_multimodality call is commented out in evaluation()

    # device setup
    device_id = 0  # Adjusted to 0 for single GPU
    ...
    mm_num_times = 10
    diversity_times = 300
    replication_times = 1  # set to 1 as long as the model is deterministic

    # gt dataset and loader
    split_file = "test.txt"
    print("Using split file:", split_file)
    gt_loader, gt_dataset = get_dataset_motion_loader(
        dataset_opt_path, batch_size, device, _split_file=split_file
    )

    # evaluation motion loaders setup
    eval_motion_loaders = {
        "MotionQwen": lambda: (
            # model motion loader
            get_qwen_model_loader(model, gt_loader, device),
            # multimodality motion loader (set to None for deterministic model)
            None,
        )
    }
    ...
    log_file = "./output/eval_results/evaluation.log"
    evaluation(log_file)
